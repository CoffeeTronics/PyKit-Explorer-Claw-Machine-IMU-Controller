...
class IMUSensor:
    ...
    @property
    def euler_angles_game(self) -> tuple:
        """Euler angles (roll, pitch, yaw) in degrees computed from game quaternion.

        Uses gyro + accelerometer fusion without magnetometer, avoiding magnetic
        interference. Yaw will drift slowly over time but is more stable for
        gaming and motion control applications.

        Requires enable_game_rotation_vector() to be called first.
        Returns (roll, pitch, yaw) tuple.
        """
        qw, qx, qy, qz = self._bno.game_quaternion
